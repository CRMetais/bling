...
# true  = só mostra o payload, NÃO envia pro Bling
# false = envia pro Bling
MODO_PREVIEW = True
...
# mapeamento dos dados pra enviar pro bling
...
def enviar_para_bling(nf, token):
    try:
        ...
        return {"status": response.status_code, "resposta": response.json() if response.content else {}}
...
def receber_nf():
    try:
        ...
        if MODO_PREVIEW:
            return jsonify({"status": "preview", "payload_bling": payload})
